...
const getValidationSchema = (mode) => {
    ...
    // Only add password validation for Add mode
    if (mode === "Add") {
...
const getInitialValues = (mode, userDetails) => {
...
const AddEditUserComponent = ({ open, onClose, onUserAdded, mode = "Add", userDetails = null }) => {
    ...
    const handleSubmit = async (values, { setSubmitting, resetForm }) => {
        try {
            ...
            if (mode === "Edit") {
                // For edit mode, include the user ID and exclude password if empty
                requestData = {
                ...
                // For add mode, include all fields including password
                requestData = values;
            ...
            console.error(`Error ${mode.toLowerCase()}ing user:`, err);
            setError(err.message || `Failed to ${mode.toLowerCase()} user`);
        } finally {
            setLoading(false);
            setSubmitting(false);
        }
    ...
    return (
        ...
        >
            ...
            <DialogContent sx={{ p: 3 }}>
                ...
                <Formik
                    initialValues={getInitialValues(mode, userDetails)}
